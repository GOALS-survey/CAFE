...
#################################
def grain_radii(numRad=81, minRad=1e-3, maxRad=10.0):
...
def grain_crosssection(wave, rad, scaleSIL, tablePath):
    ...
    # uses table header values (241 pts, 0.001–1000 µm)
    waveTab = np.logspace(-3, 3, num=241)

    # uses table header values (91 pts, 0.000316–10 µm)
    aTab = np.geomspace(0.00031622777, 10.0, 91)
    ...
    ### off-grid, up to about 4 percent, typically in ~0.05 percent.
    ...
    q_Gra = 0.01
    a_Xi = 50.0
    xi_PAH = (1.0 - q_Gra) * (a_Xi * 1e-4 / rad) ** 3
    xi_PAH[(a_Xi * 1e-4 / rad) ** 3 > 1] = 1.0 - q_Gra
    xi_PAH = np.tile(xi_PAH, (wave.size, 1))
    cAbsGraCarb = (1.0 - xi_PAH) * cAbsGra
    cExtGraCarb = (1.0 - xi_PAH) * cExtGra
    ...
    cExt = {
        ...
        "Sil": cExtSil,
        "Gra": cExtGra,
        "CarbNeu": [cExtNeuCont, cNeuFeat],
        "CarbIon": [cExtIonCont, cIonFeat],
    }
    cAbs = {
        "wave": wave,
        "a": rad,
        "Sil": cAbsSil,
        "Gra": cAbsGra,
        "CarbNeu": [cAbsNeuCont, cNeuFeat],
        "CarbIon": [cAbsIonCont, cIonFeat],
    }
...
def grainEQTemp(
    a, T_bb, sourceType, tablePath, scaleSIL=None, cAbs=None, TTable=None
):
    """
    ...
    if (
        type(sourceType) == str
    ):  # Why type(sourceType)==str?? This makes ISRF to be the only case.
        # Load an ISRF sed if unspecified
        # sourceType = 'ISRF'
        if TTable is None:
            print("Generating profiles for", sourceType, "SED profile")
    ...
    if TTable is None:
        ...
        bigPlanck = np.tile(planck(wave, T), (a.size, 1, 1)).transpose(
            [2, 1, 0]
        )
        intSil *= bigPlanck
        intCarb *= bigPlanck
    ...
    return (
        {"rad": a, "T": T_bb, "Sil": TSil, "Carb": TCarb},
        cAbs,
        TTable,
    )  # old
...
def grainSizeDF(
    ...
    if betGra > 0:
        dndaGra0 *= 1 + betGra * a / atGra
    else:
        dndaGra0 /= 1 - betGra * a / atGra
    dndaSil0 = (cSil / a) * np.power(a / atSil, -1.0 * alpSil)
    if betSil > 0:
        dndaSil0 *= 1 + betSil * a / atSil
    else:
        dndaSil0 /= 1 - betSil * a / atSil

    # Apply large grain-size cutoff
    dndaGra0[a > atGra] *= np.exp(
        -1.0 * np.power(((a[a > atGra] - atGra) / acGra), gamMax)
    )
    dndaSil0[a > atSil] *= np.exp(
        -1.0 * np.power(((a[a > atSil] - atSil) / acSil), gamMax)
    )
    # Apply absolute min and max grain size cutoffs
    dndaGra0[a < 1e-4 * aMin] = 0.0
    dndaSil0[a < 1e-4 * aMin] = 0.0
    dndaGra0[a > aMax] = 0.0
    dndaSil0[a > aMax] = 0.0

    # Calculate VSG distributio function
    mC = 1.99e-23  # Carbon atmoic mass [g]
    a0i = np.asarray([3.5, 30.0])  # Log-normal peak locations [A]
    sigma = 0.4  # Log-normal width
    bCi = np.asarray([0.75, 0.25])  # Fraction of bC in each peak
    densGra = 2.24  # Graphide density in [g cm-3]
    cmPerA = 1e-8
    umPerA = 1e-4
    dndaVSG0 = 0.0 * dndaGra0
    if bC > 0:
        B = (
            3
            / np.sqrt(2 * np.pi) ** 3
            * np.exp(-4.5 * sigma**2)
            / (densGra * (cmPerA * a0i) ** 3 * sigma)
            * (1e-6 * bC)
            * bCi
            * mC
            / (
                ...
                + erf(
                    3 * sigma / np.sqrt(2)
                    + np.sqrt(2) * np.log(a0i / 3.5) / sigma
                )
            )
        )
        dndaVSG0 = (B[0] / a) * np.exp(
            -0.5 * (np.log(a / (umPerA * a0i[0])) / sigma) ** 2
        ) + (B[1] / a) * np.exp(
            -0.5 * (np.log(a / (umPerA * a0i[1])) / sigma) ** 2
        )
        dndaVSG0[a < 1e-4 * aMin] = 0.0
        dndaVSG0[a > aMax] = 0.0
    ...
    for i in range(T_bb.size):
        ...
        if T_bb.size == 1:
            dndaSil[tEQ["Sil"] > tSubSil, i] = 0.0
            dndaCarb[tEQ["Carb"] > tSubSil, i] = 0.0
        else:
            dndaSil[tEQ["Sil"][:, i] > tSubSil, i] = 0.0
            dndaCarb[tEQ["Carb"][:, i] > tSubSil, i] = 0.0
...
def grain_opacity(
    ...
    scaleSIL -- table that stored in ohmc_scale.txt file
    ...
    if wave is None:
        sys.exit(
            "No wavelength array provided for grain_emissivity calculations."
        )
    logWave = np.log(wave)
    # waveRed = np.geomspace(wave[0], wave[-1], num=1000) # wavelength upsampling
    waveRed = np.geomspace(min(1.0, wave[0]), max(1e3, wave[-1]), num=1000)
    ...
    if dnda is None:
        dnda = grainSizeDF(
            a, T_bb, None, tablePath, scaleSIL=scaleSIL, cutoff=cutoff
        )  # None is given to imply there is no sourceType

    # Calculate total silicate opacities
    bigDndaSil = np.tile(
        dnda["Sil"][:, 0], (waveRed.size, T_bb.size)
    )  # Note: only consider dnda with T_bb=3 K.
    ...
    if kExt:
        kExtSilTot = intTab(
            intSil.transpose() * cExt_wR["Sil"].transpose(), dloga
        )

    # Calculate carbonaceous opacities
    bigDndaCarb = np.tile(
        dnda["Carb"][:, 0], (waveRed.size, T_bb.size)
    )  # Note: only consider dnda with T_bb=3 K.
    ...
    denSil = 3.5e-12
    bMass = np.tile(
        4 * np.pi / 3 * denSil * a**3, (waveRed.size, 1)
    ).transpose()
    ...
    if kExt:
        ...
        return (
            kAbsOut,
            kExtOut,
        )  # absorption and extinction=absorption+scattering
    else:
        return kAbsOut


def grain_emissivity(
    wave,
    T_bb,
    sourceType,
    scaleSIL,
    ...
    if T_EQ is None:
        ...
        T_EQ, cAbs, TTable = grainEQTemp(
            a, T_bb, sourceType, tablePath, scaleSIL=scaleSIL
        )  # old
    # ---
    ...
    for i in range(T_bb.size):
        planckSil[:, :, i] = planck(
            waveRed, T_EQ["Sil"][:, i], um=False
        ).transpose()  # Check units
        planckCarb[:, :, i] = planck(
            waveRed, T_EQ["Carb"][:, i], um=False
        ).transpose()
    ...
    eSilTot = intTab(
        ...
        * bigDndaSil
        * planckSil
        * np.tile(cSil, (T_bb.size, 1, 1)).transpose(1, 2, 0),
        dloga,
    )
    # Calculate carbonaceous emissivities
    bigDndaCarb = np.tile(dnda["Carb"], (waveRed.size, 1, 1)).transpose(
        [1, 0, 2]
    )
    eCarb = intTab(
        biga
        * bigDndaCarb
        * planckCarb
        * np.tile(cCarb, (T_bb.size, 1, 1)).transpose(1, 2, 0),
        dloga,
    )
    ...
    for i in range(len(T_bb)):
        if np.any(eSilTot > 0.0):
            ...
            eSilTotTemp[eSilTotTemp <= 0.0] = (
                1e-200  # to prevent underflow funkiness
            )
    ...
    cEns = bMass * cEns0
    eFstFeat = intTab(
        np.tile(cFst, (T_bb.size, 1, 1)).transpose(1, 2, 0), dloga
    )
    eEnsFeat = intTab(
        np.tile(cEns, (T_bb.size, 1, 1)).transpose(1, 2, 0), dloga
    )
...
def init_interpolator(E_T=None, fCarb=1.0, fSil=1.0, fCry=0.0, fEns=0.5):
    if E_T is None:
...
def compute_tau_crossover_table(waveMod, E_T, kAbs, fCarb=1.0, fSil=1.0):
    """Pre-compute τ_crossover(T_bb) — the 9.7 µm optical depth at which
    silicate grain emissivity and screen absorption cancel exactly, leaving
    the feature neither in emission nor absorption (Kirchhoff self-cancellation).

    The analytical result is:

        τ_crossover(T_bb) = S_0(T_bb) / (1 − k_c)

    where
        S_0(T_bb)  = max(ln(j_0(9.7, T_bb) / j_0_cont(9.7, T_bb)), 0)
        k_c        = (kAbs["Carb"] + kAbs["SilCont"])[9.7 µm] / kAbsTot0
        kAbsTot0   = max of (kAbs["Carb"] + kAbs["SilAmoTot"]) in 8.5–11.0 µm
                     (same normalisation convention as lib.py)

    The table is indexed by the WRM radiation-field temperature T_bb and is
    designed to be called once at initialisation (inside make_cont_profs) and
    then used via np.interp() inside the PAH extinction block of lib.py,
    replacing the per-iteration root-find that would otherwise be needed.

    Parameters
    ----------
    waveMod : ndarray, shape (N_wave,)
        Model wavelength array [µm]; used for kAbs interpolation.
    E_T : dict
        Grain emissivity dictionary returned by grain_emissivity(); must
        contain keys 'wave', 't_bb', 'Carb', 'SilTot', 'SilCont'.
    kAbs : dict
        Grain absorption opacity dictionary; must contain keys 'Carb',
        'SilAmoTot', 'SilCont'.
    fCarb : float, optional
        Carbonaceous grain scaling factor (default 1.0).
    fSil : float, optional
        Silicate grain scaling factor (default 1.0).

    Returns
    -------
    dict
        'T_bb' : ndarray, shape (N_Tbb,)  — radiation-field temperatures [K]
        'tau'  : ndarray, shape (N_Tbb,)  — τ_crossover at each T_bb
    """
    # ── continuum opacity normalisation (same as lib.py) ─────────────────────
    kAbsTot_full = kAbs["Carb"] + kAbs["SilAmoTot"]
    idxMax = (waveMod > 8.5) & (waveMod < 11.0)
    kAbsTot0 = np.nanmax(kAbsTot_full[idxMax])

    # k_c: normalised continuum-only opacity at 9.7 µm (no silicate resonance)
    kAbs_cont = kAbs["Carb"] + kAbs["SilCont"]
    k_c = float(np.interp(9.7, waveMod, kAbs_cont) / kAbsTot0)

    # ── loop over temperature grid ────────────────────────────────────────────
    wave_ET = E_T["wave"]          # emissivity wavelength grid
    n_Tbb = E_T["t_bb"].size
    tau_xover = np.zeros(n_Tbb)

    for i in range(n_Tbb):
        # Total emissivity includes amorphous silicate resonance feature
        j0_total = fCarb * E_T["Carb"][:, i] + fSil * E_T["SilTot"][:, i]
        # Continuum emissivity: no amorphous silicate feature
        j0_cont  = fCarb * E_T["Carb"][:, i] + fSil * E_T["SilCont"][:, i]

        # Clip negatives before interpolation
        j0_total = np.clip(j0_total, 1e-300, None)
        j0_cont  = np.clip(j0_cont,  1e-300, None)

        # Interpolate to 9.7 µm
        j0_97      = float(np.interp(9.7, wave_ET, j0_total))
        j0_cont_97 = float(np.interp(9.7, wave_ET, j0_cont))

        # S_0: intrinsic silicate feature strength (≥ 0 by construction)
        S0 = max(np.log(j0_97 / j0_cont_97), 0.0) if j0_cont_97 > 0 else 0.0

        # τ_crossover — guard against degenerate k_c ≈ 1
        denom = 1.0 - k_c
        tau_xover[i] = S0 / denom if denom > 1e-6 else 0.0

    return {"T_bb": E_T["t_bb"].copy(), "tau": tau_xover}


def grain_totemissivity(
    wave,
    T_bb,
    fCarb=1.0,
    fSil=1.0,
    ...
    if FASTTEMP:
        ...
        j = (
            fCarb * E_T["Carb"][:, arglow : arghigh + 1]
            + fSil * ((1 - fCry) * E_T["SilTot"][:, arglow : arghigh + 1])
            + fCry
            * (
                (1 - fEns) * E_T_Sil
                + fEns * (E_T_Sil + E_T["EnsFeat"][:, arglow : arghigh + 1])
            )
        )
        j[j < 0] = 1e-200
        ...
        if not (
            wave.shape == E_T["wave"].shape and np.allclose(wave, E_T["wave"])
        ):
            assert 1 == 0
...
def pah_crosssection(wave, a, cGra, feat=False):
    ...
    if not feat:
        drudeC = np.asarray(
            [
                [7.220e-2, 0.195, 7.97e7, 7.97e7],
                [2.175e-1, 0.217, 1.23e7, 1.23e7],
                [1.050e0, 0.055, 0.00e0, 2.00e4],
                [1.260e0, 0.110, 0.00e0, 7.8e-2],
                [1.905e0, 0.090, 0.00e0, -146.5],
                [1.500e1, 0.800, 5.00e1, 5.00e1],
            ]
        )
        peakNeuC = (
            2.0
    ...
    drudeF = np.asarray(
        [
            [3.300e0, 0.012, 394.0, 89.4],
            [5.250e0, 0.030, 2.5, 20.0],
            [5.700e0, 0.040, 7.5, 60.0],
            [6.220e0, 0.0284, 29.4, 236.0],
            [6.690e0, 0.070, 5.88, 47.2],
            [7.417e0, 0.126, 15.8, 142.0],
            [7.598e0, 0.044, 23.8, 214.0],
            [7.850e0, 0.053, 21.3, 192.0],
            [8.330e0, 0.052, 6.94, 48.4],
            [8.610e0, 0.039, 27.8, 194.0],
            [1.123e1, 0.010, 12.8, 12.0],
            [1.130e1, 0.029, 58.4, 54.7],
            [1.199e1, 0.050, 24.2, 20.5],
            [1.261e1, 0.0435, 34.8, 31.0],
            [1.360e1, 0.020, 0.5, 0.5],
            [1.419e1, 0.025, 0.5, 0.5],
            [1.590e1, 0.020, 0.5, 0.5],
            [1.6447e1, 0.014, 0.75, 0.75],
            [1.7038e1, 0.065, 3.08, 3.08],
            [1.7377e1, 0.012, 0.28, 0.28],
            [1.7873e1, 0.016, 0.14, 0.14],
        ]
    )
    ...
    HtoC[nC < 25.0] = 0.5
    HtoC[((nC > 25.0) & (nC < 100.0))] = (
        0.5 / np.sqrt(nC[((nC > 25.0) & (nC < 100.0))]) / 25.0
    )
    HtoC[nC > 100.0] = 0.25
    ...
    if idx.size > 0:
        cont = (
            bigNC[idx]
            * np.tile(
                (126.0 - 6.4934 * x[idx]) * 1e-18, (a.size, 1)
            ).transpose()
        )
        cContNeu[idx] = cont
        cContIon[idx] = cont
    ...
    idx = (x > 10) & (x < 15)
    if idx.size > 0:
        drudeFluxNeu = drude_prof(
            wave[idx], [[drudeC[0][0]], [drudeC[0][1]], [peakNeuC[0]]]
        )
        drudeFluxIon = drude_prof(
            wave[idx], [[drudeC[0][0]], [drudeC[0][1]], [peakIonC[0]]]
        )
        cont = (-3.0 + 1.35 * x[idx]) * 1e-18
        cContNeu[idx] = (
            bigNC[idx] * np.tile(drudeFluxNeu + cont, (a.size, 1)).transpose()
        )
        cContIon[idx] = (
            bigNC[idx] * np.tile(drudeFluxIon + cont, (a.size, 1)).transpose()
        )
    ### 7.7 < x < 10 ###
    idx = (x > 7.7) & (x < 10)
    cont = (
    ...
    idx = (x > 5.9) & (x < 7.7)
    drudeFluxNeu = drude_prof(
        wave[idx], [[drudeC[1][0]], [drudeC[1][1]], [peakNeuC[1]]]
    )
    drudeFluxIon = drude_prof(
        wave[idx], [[drudeC[1][0]], [drudeC[1][1]], [peakIonC[1]]]
    )
    cont = (
        1.8687
        + 0.1905 * x[idx]
    ...
    cContNeu[idx] = (
        bigNC[idx] * np.tile(cont + drudeFluxNeu, (a.size, 1)).transpose()
    )
    cContIon[idx] = (
        bigNC[idx] * np.tile(cont + drudeFluxIon, (a.size, 1)).transpose()
    )
    ### 3.3 < x < 5.9 ###
    idx = (x > 3.3) & (x < 5.9)
    drudeFluxNeu = drude_prof(
    ...
    if idx.size > 0:
        for i in range(a.size):
            ...
            sigIonF[8:14] *= HtoC[i]
            peakNeuF = (
                (2.0 / np.pi) * drudeF[:, 0] / 1e4 * sigNeuF / drudeF[:, 1]
            )
            peakIonF = (
                (2.0 / np.pi) * drudeF[:, 0] / 1e4 * sigIonF / drudeF[:, 1]
            )
            drudeFluxNeu = drude_prof(
                wave[idx], [[drudeF[:, 0], drudeF[:, 1], peakNeuF]]
            )
            drudeFluxIon = drude_prof(
                wave[idx], [[drudeF[:, 0], drudeF[:, 1], peakIonF]]
            )
            cFeatNeu[idx, i] = nC[i] * drudeFluxNeu
            cFeatIon[idx, i] = nC[i] * drudeFluxIon
            if not feat:
                cCont = 34.58 * 10 ** (-1.0 * (18.0 + (3.431 / x[idx])))
                m = 0.4 * nC[i] if nC[i] > 40.0 else 0.3 * nC[i]
                waveCutNeu = 1.0 / ((3.804 / np.sqrt(m)) + 1.052)
                waveCutIon = 1.0 / ((2.282 / np.sqrt(m)) + 0.889)
                yNeu = waveCutNeu / wave[idx]
                yIon = waveCutIon / wave[idx]
                cutNeu = (
                    np.arctan(1e3 * (yNeu - 1.0) ** 3 / yNeu) / np.pi
                ) + 0.5
                cutIon = (
                    np.arctan(1e3 * (yIon - 1.0) ** 3 / yIon) / np.pi
                ) + 0.5
                drudeFluxNeu = drude_prof(
                    wave[idx], [[drudeC[5][0]], [drudeC[5][1]], [peakNeuC[5]]]
                )
                drudeFluxIon = drude_prof(
                    wave[idx], [[drudeC[5][0]], [drudeC[5][1]], [peakIonC[5]]]
                )
    ...
    if not feat:
        drudeFluxNeu = drude_prof(
            wave, [drudeC[:, 0][2:5], drudeC[:, 1][2:5], peakNeuC[2:5]]
        )
        drudeFluxIon = drude_prof(
            wave, [drudeC[:, 0][2:5], drudeC[:, 1][2:5], peakIonC[2:5]]
        )
        cCont = 3.5 * 10 ** (-19.0 - (1.45 / x)) * np.exp(-0.1 * x**2)
        cContNeu += bigNC * np.tile(drudeFluxNeu, (a.size, 1)).transpose()
        cContIon += (
            bigNC * np.tile(drudeFluxIon + cCont, (a.size, 1)).transpose()
        )
