...
class cafe_io:
    ...
    def customFITSReader(file_name, flux_key="Flux_st", flux_unc_key="Err_st"):
        ...
        if len(hdu_list[1].data) == 1:
            res_spec1d = res_spec1d[0]
        ...
        if instnames[0] != "PHOTOMETRY":
            ...
            for inst in list(map(str.upper, instnames)):
                if any(inst in file for file in files):
                    ...
                    raise IOError(
                        "One or more resolving-power files not in directory. Or check the names."
                    )
            ...
            for inst_fn in inst_files:
                try:
                    ...
                    try:
                        ...
                        data[3] = (
                            rfitstable[1].data[-1][2] - rfitstable[1].data[0][2]
                        ) / (
                            rfitstable[1].data[-1][0] - rfitstable[1].data[0][0]
                        )  # R_s
                        data[4] = (
                            rfitstable[1].data[-1][2]
                            - data[3] * rfitstable[1].data[-1][0]
                        )  # Bias
                        rfitstable.close()
                    except:
                        raise Exception(
                            "Resolving power table format not recognized"
                        )
    ...
    def write_inifile(params, opts, fname):
        ...
        for par in params:
            ...
            try:
                ...
                if expr is not None:
                    opts["FITTED PARAM VALUES AND OPTIONS"][par] = [
                        val,
                        vary,
                        low,
                        high,
    ...
    def pah_table(
        ...
        if (compdict is None) & (pah_obs is True):
            warnings.warn(
                "pah_obs is set True; however, no compdict is provided. Thus, observed PAH flux cannot be calculated."
            )
        ...
        for n in pah_name:
            ...
            pah_strength = (np.pi * const.c.to("micron/s") / 2) * (
                peak * gamma / lam
            )  # * 1e-26 # * u.watt/u.m**2
            ...
            if compdict is not None:
                ...
                if pah_obs is True:
                    ...
                    ext_scale = np.interp(
                        lam.value, wave, compdict["extComps"]["extPAH"]
                    )

                    pah_strength_obs_list.append(
                        pah_strength.to(u.Watt / u.m**2).value * ext_scale
                    )
            ...
            if (
                (_lam_unc is not None)
                & (_gamma_unc is not None)
                & (_peak_unc is not None)
            ):
                ...
                g_over_lam_unc = (
                    gamma
                    / lam
                    * np.sqrt((gamma_unc / gamma) ** 2 + (lam_unc / lam) ** 2)
                )  # uncertainty of gamma/lam

                pah_strength_unc = (
                    (np.pi * const.c.to("micron/s") / 2)
                    * (peak * gamma / lam)
                    * np.sqrt(
                        (peak_unc / peak) ** 2
                        + (g_over_lam_unc / (gamma / lam)) ** 2
                    )
                )  # * 1e-26# * u.watt/u.m**2

                # Make the unit appear as W/m^2
                pah_strength_unc_list.append(
                    pah_strength_unc.to(u.Watt / u.m**2).value
                )

                if (compdict is not None) & (pah_obs is True):
                    pah_strength_obs_unc_list.append(
                        pah_strength_unc.to(u.Watt / u.m**2).value * ext_scale
                    )
            else:
            ...
            if compdict is not None:
                ...
                # Continuum profile
                I_nu_C = compdict["CompFluxes"][
                    "fCON"
                ]  # The straightout continuum flux level, without extinction correction

                # PAH profile
                I_nu_P = (
                    drude_prof(wave, [[lam.value], [gamma], [peak.value]])
                    + I_nu_C
                )
        ...
        if (compdict is None) & (pah_obs is False):  # No EQW can be provided
        ...
        elif (compdict is not None) & (
            pah_obs is False
        ):  # Output the extinction corrected PAH fluxes with the EQW
            all_pah_df = pd.DataFrame(
                {
                    "pah_name": pah_name,
                    "pah_lam": pah_lam_list,
            ...
            pah_obs is True
        ):  # Output the extinction corrected and observed PAH fluxes with the EQW
            all_pah_df = pd.DataFrame(
                {
                    "pah_name": pah_name,
                    "pah_lam": pah_lam_list,
        ...
        for pah_lam in all_pah_df.pah_lam:
            ...
            for mainpah_key in mainpah_dict.keys():
                if (pah_lam >= mainpah_dict[mainpah_key]["range"][0]) & (
                    pah_lam < mainpah_dict[mainpah_key]["range"][1]
                ):
        ...
        pah_complex_strength = all_pah_df.groupby("pah_complex")[
            "pah_strength_int"
        ].sum()

        pah_complex_strength_unc = all_pah_df.groupby("pah_complex")[
            "pah_strength_int_unc"
        ].apply(lambda x: np.sqrt(np.sum(x**2)))

        if pah_obs is False:
            pah_complex_df = pd.concat(
                [pah_complex_strength, pah_complex_strength_unc], axis=1
            )
        else:
            pah_complex_strength_obs = all_pah_df.groupby("pah_complex")[
                "pah_strength_obs"
            ].sum()

            pah_complex_strength_obs_unc = all_pah_df.groupby("pah_complex")[
                "pah_strength_obs_unc"
            ].apply(lambda x: np.sqrt(np.sum(x**2)))

            pah_complex_df = pd.concat(
                [
                    pah_complex_strength,
                    pah_complex_strength_unc,
                    pah_complex_strength_obs,
                    pah_complex_strength_obs_unc,
                ],
                axis=1,
            )
        ...
        if compdict is not None:
            ...
            def find_closest_limits(elements, limits):
                ...
                if lower_index > 0 and (
                    lower_limit - elements.iloc[lower_index - 1]
                    < elements.iloc[lower_index] - lower_limit
                ):
                    lower_index -= 1

                # Check if elements are within the limits
                closest_to_lower = (
                    elements.iloc[lower_index]
                    if lower_limit <= elements.iloc[lower_index] <= upper_limit
                    else None
                )
                closest_to_upper = (
                    elements.iloc[upper_index - 1]
                    if upper_index > 0
                    and lower_limit
                    <= elements.iloc[upper_index - 1]
                    <= upper_limit
                ...
                if (
                    closest_to_lower == closest_to_upper
                    or closest_to_upper is None
                ):
            ...
            for mainpah_key in mainpah_dict.keys():
                pah_element = find_closest_limits(
                    all_pah_df.pah_lam, mainpah_dict[mainpah_key]["range"]
                )
                ...
                if pah_element is None:
                    ...
                    gamma = all_pah_df_tmp[
                        all_pah_df_tmp.pah_lam == pah_element[0]
                    ].pah_gamma.values[0]
                    lam = all_pah_df_tmp[
                        all_pah_df_tmp.pah_lam == pah_element[0]
                    ].pah_lam.values[0]
                    ...
                    for i in range(len(all_pah_df_tmp.pah_lam)):
                        ...
                        if (lam >= mainpah_dict[mainpah_key]["range"][0]) & (
                            lam < mainpah_dict[mainpah_key]["range"][1]
                        ):
                            wave = prof_df.wave
                            ...
                            # PAH profile
                            I_nu_Pi = (
                                drude_prof(wave, [[lam], [gamma], [peak]])
                                + I_nu_C
                            )

                    prof_df["I_nu_P"] = I_nu_Pi
                    pah_range = prof_df[
                        (prof_df.wave >= pah_comp_wmin)
            ...
            pah_complex_eqw_df = pd.DataFrame(
                {
                    "pah_complex": list(mainpah_dict.keys()),
                    "pah_complex_eqw": pah_complex_eqw_list,
                }
            ).set_index("pah_complex")

            pah_complex_df = pd.merge(
                pah_complex_df,
                pah_complex_eqw_df,
                how="inner",
                left_index=True,
    ...
    def line_table(
        ...
        if (compdict is None) & (line_obs is True):
            warnings.warn(
                "line_obs is set True; however, no compdict is provided. Thus, observed line flux cannot be calculated."
            )

        line_parname = [i[0] == "g" for i in df.index]
        # line_name = list(set([n.split('_')[1] for n in df[line_parname].index]))
        line_name = list(
            set(["_".join(n.split("_")[1:3]) for n in df[line_parname].index])
        )
        ...
        for n in line_name:
            ...
            if compdict is not None:
                ...
                if line_obs is True:
                    ...
                    ext_scale = np.interp(
                        lam.value, wave, compdict["extComps"]["extPAH"]
                    )  # line and PAH subjected to same level of extinction

                    line_strength_obs_list.append(
                        line_strength.to(u.Watt / u.m**2).value * ext_scale
                    )
            ...
            if (
                ...
                # Make the unit appear as W/m^2
                line_strength_unc_list.append(
                    line_strength_unc.to(u.Watt / u.m**2).value
                )

                if (compdict is not None) & (line_obs is True):
                    line_strength_obs_unc_list.append(
                        line_strength_unc.to(u.Watt / u.m**2).value * ext_scale
                    )
            else:
        ...
        if (compdict is None) | (
            line_obs is False
        ):  # Output the extinction corrected line fluxes
            ...
            line_obs is True
        ):  # Output the extinction corrected and observed PAH fluxes
            all_line_df = pd.DataFrame(
                {
                    "line_name": line_name,
                    "line_lam": line_lam_list,
        ...
        columns_to_update = [
            col for col in all_line_df.columns if "strength" in col
        ]

        all_line_df[columns_to_update] = all_line_df[
            columns_to_update
        ] * fnu_unit.to(u.Jy)
    ...
    def save_asdf(cafe, pah_tbl=True, line_tbl=True, file_name=None, **kwargs):
        ...
        try:
            flux, CompFluxes, CompFluxes_0, extComps, e0, tau0, _ = (
                get_model_fluxes(
                    fitPars, wave, cafe.cont_profs, verbose_output=True
                )
            )
        except:
        ...
        output_gauss = {
            ...
            "peak": gauss[2],
            "name": gauss[3],
            "strength": np.sqrt(2.0 * np.pi) * 2.998e5 * gauss[1] * gauss[2],
