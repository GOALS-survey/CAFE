...
class CAFE_param_generator:
    ...
    def make_parobj(
        ...
        """
        Build the lmfit Parameters object based on the input spectrum and two
        parameter types (inpars and inopts).
        This function combines the Parameter objects output from "make_cont_pars"
        and "make_feat_pars".
        ...
        try:
            params = self.make_cont_pars(
                ...
                Onion=inopts["SWITCHES"]["ONION"],
            )
        except Exception as exc:
            raise IOError("Input parameter file not found") from exc

        # Note that if a parameter object (= parobj_update) is provided,
        # the continuum parameters will still have the original limits
        ...
        if inpars["MODULES & TABLES"]["RESOLUTIONS"] != "":
            ...
            if isinstance(instnames, str):
                instnames = [
                    instnames
                ]  # raise ValueError('The instrument/module names is not a list')
        else:
            raise IOError("No spectral modules specified")
        self.instnames = instnames

        if updated_parobj is False:
            ...
            gauss, drude, gauss_opc = self.init_feats(
                self.wave,
                self.flux,
                self.flux_unc,
                self.z,
            ...
            gauss, drude, gauss_opc = self.get_feats(
                updated_parobj
            )  # We do NOT apply VGRAD to the wavelengths
        ...
        feat_params = self.make_feat_pars(
            inpars["PAH & LINE OPTIONS"],
            gauss,
            drude,
            gauss_opc,
            get_all=get_all,
        ...
        if (
            ...
            * 3
            ...
            raise ValueError(
                "There has been a rejection during the creation of the parameters, which should not have happened"
            )
        ...
        if get_all is True:
            print(
                ...
                int((len(feat_params) - 1) / 3)
                - sum(gauss[4] == 1)
                - (len(drude[0]) + len(gauss_opc[0])),
                "lines (of which",
                sum(gauss[4] == 1),
                "have double components),",
                len(drude[0]),
                "PAHs,",
                len(gauss_opc[0]),
                "extra opacity features,",
                len(params) - len(feat_params),
                "continuum parameters, and the VGRAD parameter",
            )
        else:
            print(
                "The parameter object has",
                int((len(feat_params) - 1) / 3)
                - sum((gauss[2] > 0) & (gauss[4] == 1))
                - (len(drude[0]) + len(gauss_opc[0])),
                "lines (of which",
                sum((gauss[2] > 0) & (gauss[4] == 1)),
                "have double components),",
            ...
            if inopts["FIT OPTIONS"]["FITLINS"] is False:
                print("Note that lines have been set to NOT be fitted")
            if inopts["FIT OPTIONS"]["FITPAHS"] is False:
                print("Note that PAHs have been set to NOT be fitted")
            if inopts["FIT OPTIONS"]["FITOPCS"] is False:
                print(
                    "Note that Gauss opacities have been set to NOT be fitted"
                )
    ...
    def init_feats(
        ...
        # ---------------------
        data = np.genfromtxt(
            os.path.join(tablePath, atomic_table), comments=";", dtype="str"
        )
        if data.ndim == 1:
        ...
        for i in range(len(inst_df)):
            idx = (
                (aMask == 0)
                & (aWave0 * (1 + z) > inst_df.iloc[i].wMin)
                & (aWave0 * (1 + z) < inst_df.iloc[i].wMax)
                & (aWave0 >= minWave)
                & (aWave0 <= maxWave)
            )
            aGam = np.concatenate(
                (
                    ...
                    1.0
                    / (
                        inst_df.iloc[i].rSlope * aWave0[idx] * (1 + z)
                        + inst_df.iloc[i].rBias
                    ),
                )
            )
            aNames_kept, aNames = (
                np.concatenate((aNames_kept, aNames[idx])),
                aNames[~idx],
            )
            aWave0_kept, aWave0 = (
                np.concatenate((aWave0_kept, aWave0[idx])),
                aWave0[~idx],
            )
            aDoub_kept, aDoub = (
                np.concatenate((aDoub_kept, aDoub[idx])),
                aDoub[~idx],
            )
            aMask = np.delete(aMask, idx, 0)

        for i, name in enumerate(aNames_kept):
            aNames_kept[i] = (
                name.replace(")", "")
                .replace("(", "")
                .replace("-", "")
        ...
        # ------------------------
        data = np.genfromtxt(
            os.path.join(tablePath, molecular_table), comments=";", dtype="str"
        )
        if data.ndim == 1:
            data = np.expand_dims(data, 0)
        ...
        for i in range(len(inst_df)):
            idx = (
                (mMask == 0)
                & (mWave0 * (1 + z) > inst_df.iloc[i].wMin)
                & (mWave0 * (1 + z) < inst_df.iloc[i].wMax)
                & (mWave0 >= minWave)
                & (mWave0 <= maxWave)
            )
            mGam = np.concatenate(
                (
                    ...
                    1.0
                    / (
                        inst_df.iloc[i].rSlope * mWave0[idx] * (1 + z)
                        + inst_df.iloc[i].rBias
                    ),
                )
            )
            mNames_kept, mNames = (
                np.concatenate((mNames_kept, mNames[idx])),
                mNames[~idx],
            )
            mWave0_kept, mWave0 = (
                np.concatenate((mWave0_kept, mWave0[idx])),
                mWave0[~idx],
            )
            mDoub_kept, mDoub = (
                np.concatenate((mDoub_kept, mDoub[idx])),
                mDoub[~idx],
            )
            mMask = np.delete(mMask, idx, 0)

        for i, name in enumerate(mNames_kept):
            mNames_kept[i] = (
                name.replace(")", "")
                .replace("(", "")
                .replace("-", "")
        ...
        # ----------------------------
        data = np.genfromtxt(
            os.path.join(tablePath, hrecomb_table), comments=";", dtype="str"
        )
        if data.ndim == 1:
            data = np.expand_dims(data, 0)
        ...
        for i in range(len(inst_df)):
            idx = (
                (hMask == 0)
                & (hWave0 * (1 + z) > inst_df.iloc[i].wMin)
                & (hWave0 * (1 + z) < inst_df.iloc[i].wMax)
                & (hWave0 >= minWave)
                & (hWave0 <= maxWave)
            )
            hGam = np.concatenate(
                (
                    ...
                    1.0
                    / (
                        inst_df.iloc[i].rSlope * hWave0[idx] * (1 + z)
                        + inst_df.iloc[i].rBias
                    ),
                )
            )
            hNames_kept, hNames = (
                np.concatenate((hNames_kept, hNames[idx])),
                hNames[~idx],
            )
            hWave0_kept, hWave0 = (
                np.concatenate((hWave0_kept, hWave0[idx])),
                hWave0[~idx],
            )
            hDoub_kept, hDoub = (
                np.concatenate((hDoub_kept, hDoub[idx])),
                hDoub[~idx],
            )
            hMask = np.delete(hMask, idx, 0)

        for i, name in enumerate(hNames_kept):
            hNames_kept[i] = (
                name.replace(")", "")
                .replace("(", "")
                .replace("-", "")
        ...
        if get_all == False:
            waveMinMax = [wave0 * (1.0 - 2.0 * gam), wave0 * (1.0 + 2.0 * gam)]

            # Identify gauss features that are absent and set their peak to 0
            nrep = 3
            featflux = np.copy(flux)
            for i in range(nrep):
                fluxMinMax = np.interp(
                    waveMinMax, wave, featflux
                )  # linear interpolation at waveMinMax
        ...
        gauss = [
            np.asarray(wave0),
            np.asarray(gam),
            np.asarray(peaks * 2.0),
            names,
            np.asarray(doubs),
        ]

        flux_left = flux - gauss_prof(
            wave, gauss
        ...
        if get_all == True:
            if (
                wave0.size
                != aWave0_kept.size + mWave0_kept.size + hWave0_kept.size
            ):
                raise ValueError(
                    "Not all possible gauss features kept even when get_all=True"
                )

        # -------------
        # Get PAH table
        # -------------
        pahTab = np.genfromtxt(os.path.join(tablePath, pah_table), comments=";")
        if pahTab.ndim == 1:
        ...
        for ref_pah_wave in pah_inspection_order:
            ...
            if (ref_pah_wave >= minWave) and (ref_pah_wave <= maxWave):
                ...
                idx = (
                    np.abs(wave0PAH - ref_pah_wave)
                ).argmin()  # PAH band it's closest to the input PAH wavelength
                if instnames[0] != "PHOTOMETRY":
                    ...
                    waveMinMax = [
                        wave0PAH[idx] * (1.0 - 10000.0 / 2.998e5),
                        wave0PAH[idx] * (1.0 + 10000.0 / 2.998e5),
                    ]
                    fluxMinMax = np.interp(waveMinMax, wave, flux_left)
                    cont = 0.5 * (fluxMinMax[0] + fluxMinMax[1])
                    ref_pah_peak = (
                        np.interp(wave0PAH[idx], wave, flux_left) - cont
                    )
                    peakPAH_guess = peakPAH / peakPAH[idx] * ref_pah_peak
                    break
                else:
                    ref_pah_peak = (
                        np.interp(wave0PAH[idx], wave, flux_left) / 2.0
                    )
                    peakPAH_guess = peakPAH / peakPAH[idx] * ref_pah_peak
                    break
        ...
        if ref_pah_peak == 0.0:
            ...
            raise ValueError(
                "Input spectrum does not contain any PAH band available to perform the PAH component scaling."
            )

        namePAH = []
        for i in range(len(wave0PAH)):
            namePAH.append(
                str(int(compPAH[i]))
                + "_PAH"
                + str(round(wave0PAH[i] * 1e1))
                + "_"
                + str(round(wave0PAH[i] * 1e4))
            )

        drude = [
            np.asarray(wave0PAH),
            np.asarray(gamPAH),
            np.asarray(peakPAH_guess),
            namePAH,
            compPAH,
        ]
        ...
        # ----------------
        opcTab = Table.read(
            os.path.join(tablePath, "opacity", gopacity_table), comment="#"
        )
        ...
        for i in range(len(inst_df)):
            idx = (
                (oMask == 0)
                & (oWave0 * (1 + z) > inst_df.iloc[i].wMin)
                & (oWave0 * (1 + z) < inst_df.iloc[i].wMax)
                & (oWave0 >= minWave)
                & (oWave0 <= maxWave)
            )
            oGam_kept, oGam = np.concatenate((oGam_kept, oGam[idx])), oGam[~idx]
            oNames_kept, oNames = (
                np.concatenate((oNames_kept, oNames[idx])),
                oNames[~idx],
            )
            oWave0_kept, oWave0 = (
                np.concatenate((oWave0_kept, oWave0[idx])),
                oWave0[~idx],
            )
            oPeak_kept, oPeak = (
                np.concatenate((oPeak_kept, oPeak[idx])),
                oPeak[~idx],
            )
            oMask = np.delete(oMask, idx, 0)

        for i, name in enumerate(oNames_kept):
            oNames_kept[i] = (
                name.replace(")", "")
                .replace("(", "")
                .replace("-", "")
        ...
        gauss_opc = [
            np.asarray(oWave0_kept),
            np.asarray(oGam_kept),
            np.asarray(oPeak_kept),
            oNames_kept,
        ]
        ...
        if get_all == False:
            print(
                "Out of",
                len(inds),
                "lines set to be fitted,",
                sum(peaks == 0.0),
                "returned negative values while guessing their peaks, so they will not be fitted. These are:",
            )
        ...
        for key in pkeys:
            if key[0] == "g":  # e.g.: g_NeIII_33333B_Wave
                if key[-1] == "e":  # Wave
                    ...
                    if fwave[-1] == "B":
                        ...
                        if errors is False:
                            ...
                            lgamma.append(
                                params[key.replace("Wave", "Gamma")].value
                            )
                            lpeak.append(
                                params[key.replace("Wave", "Peak")].value
                            )
                        else:
                            lwave.append(params[key].stderr)
                            lgamma.append(
                                params[key.replace("Wave", "Gamma")].stderr
                            )
                            lpeak.append(
                                params[key.replace("Wave", "Peak")].stderr
                            )
                        lname.append(fname + "_" + fwave[0:-1])
                        # Set up a double component if there is one in the parameter object
                        (
                            ldoub.append(1)
                            if "g_" + fname + "_" + fwave[0:-1] + "B_Wave"
                            in pkeys
                            else ldoub.append(0)
                        )
                        if apply_vgrad2waves is True:
                            lwave[-1] *= 1 + params["VGRAD"] / 2.998e5
                elif key[-1] == "a" or key[-1] == "k":
                    continue
                else:
                    raise ValueError(
                        "You messed with the feature parameter names"
                    )
                ...
                if key[-1] == "e":
                    ...
                    if apply_vgrad2waves is True:
                        pwave[-1] *= 1 + params["VGRAD"] / 2.998e5
                elif key[-1] == "a" or key[-1] == "k":
                    continue
                else:
                    raise ValueError(
                ...
                if key[-1] == "e":
                    ...
                    if apply_vgrad2waves is True:
                        owave[-1] *= 1 + params["VGRAD"] / 2.998e5
                elif key[-1] == "a" or key[-1] == "k":
                    continue
                else:
                    raise ValueError(
    ...
    def make_cont_pars(
        ...
        for key in inpars.keys():
            ...
            if len(inpars[key]) == 2:
                params.add(
                    key,
                    value=inpars[key][0],
                    vary=bool(inpars[key][1]),
                    min=-np.inf,
                    max=np.inf,
                )

            # Min/max - must specify both (use np.inf if you only want one)
            elif len(inpars[key]) == 4 or (
                len(inpars[key]) == 5 and inpars[key][-1] is None
            ):
                params.add(
                    key,
                    value=inpars[key][0],
            ...
            # Tie parameters - must also set bounds (can be inf)
                ...
                try:
                    ...
                    # This happens when reloading a fit that had onion on,
                    # it confuses the expr parser
                    params.add(
                        key,
                        value=inpars[key][0],
                        vary=bool(inpars[key][1]),
            ...
            if parobj_update:
                ...
                # If the parameter has been fixed to 0 by the fitter and
                # the parameters are needed as initializers for fitting
                if (
                    params[key].value == 0
                    and params[key].vary != parobj_update[key].vary
        ...
        if Onion:
            params.add(
                "HOT_WRM",
                value=inpars["HOT_TAU"][0] / inpars["WRM_TAU"][0],
                vary=True,
                min=1.0,
                max=np.inf,
            )
            ...
            if parobj_update:
                params["HOT_WRM"].value = parobj_update["HOT_WRM"].value

            params.add(
                "WRM_COO",
                value=inpars["WRM_TAU"][0] / inpars["COO_TAU"][0],
                vary=True,
                min=1.0,
                max=np.inf,
            )
    ...
    def make_feat_pars(
        ...
        # TODO: This table should be moved to a config file
        doublets = {
            "OIII_5008": ["OIII_4960", 2.994],
            "NII_6585": ["NII_6550", 2.959],
            "OI_6366": ["OI_6302", 0.32],
            "NeIII_3969": ["NeIII_3870", 0.31],
        }
        complab = ["N", "B"]  # narrow and broad

        # When calling make_feat_pars() with parobj_update, [gauss, drude, gauss_opc]
        # have been already injected with the new updated values through get_feats()
        # instead of init_feats().
        # The latter are passed through init_parobj
        # Note that the B(broad) components even if they have been injected in
        # parobj_update, they are not in/passed to gauss, so they will be intitialized
        # with the default set up (which depends on the N components).

        # Line features
        for i in range(gauss[0].size):
            ### May eventually want to replace hardcoded values
            if gauss[2][i] > 0.0 or get_all == True or parobj_update != False:
                # Double components
                for j in range(int(gauss[4][i]) + 1):
                    ...
                    if inpars["EPSWAVE0_LIN_" + complab[j]] > 0:
                        maxW = gauss[0][i] * (
                            1.0 + inpars["EPSWAVE0_LIN_" + complab[j]] / 2.998e5
                        )  # EPSWAVE0_LIN in [km/s]
                        minW = gauss[0][i] / (
                            1.0 + inpars["EPSWAVE0_LIN_" + complab[j]] / 2.998e5
                        )
                    else:
                    ...
                    params.add(
                        "g_" + name + complab[j] + "_Wave",
                        value=gauss[0][i] / (1.0 + j * 200.0 / 2.998e5),
                        vary=inpars["FITWAVE0_LIN_" + complab[j]],
                        min=minW,
                        max=maxW,
                    )
                    if parobj_update:
                        ...
                        # pass
                        if (
                            params["g_" + name + complab[j] + "_Wave"].value
                            != parobj_update[
                                "g_" + name + complab[j] + "_Wave"
                            ].value
                        ):
                            if j == 0:
                                raise ValueError(
                                    "Gauss params and parobj_update wavelengths do not concide"
                                )  # ipdb.set_trace()
                            elif j == 1:
                                params[
                                    "g_" + name + complab[j] + "_Wave"
                    ...
                    if inpars["EPSGAMMA_LIN_" + complab[j]] > 0:
                        maxG = np.max(
                            [
                                gauss[1][i] * (1.0 + 0.05),
                                inpars["EPSGAMMA_LIN_" + complab[j]] / 2.998e5,
                            ]
                        )  # EPSGAMMA_LIN in [km/s]
                        if init_parobj is not False:
                            minG = init_parobj[
                                "g_" + name + complab[j] + "_Gamma"
                            ].value / (1.0 + 0.05)
                        else:
                            minG = gauss[1][i] / (
                                1.0 + 0.05
                            )  # Minimum allowed is set to 1/1.05 the resolution (gamma)
                    else:
                        maxG = np.inf
                        minG = gauss[1][i] / (
                            1.0 + 0.05
                    ...
                    params.add(
                        "g_" + name + complab[j] + "_Gamma",
                        value=gauss[1][i] * (2 * j + 1),
                        vary=inpars["FITGAMMA_LIN_" + complab[j]],
                        min=minG,
                        max=maxG,
                    )  # The i'th component is initialized to have a width 2*j+1 * instrumental FWHM
                    if parobj_update:
                        if (
                            params["g_" + name + complab[j] + "_Gamma"].value
                            != parobj_update[
                                "g_" + name + complab[j] + "_Gamma"
                            ].value
                        ):
                            if j == 0:
                                raise ValueError(
                                    "Gauss params and parobj_update gammas do not concide"
                                )  # ipdb.set_trace()
                            elif j == 1:
                                params[
                                    "g_" + name + complab[j] + "_Gamma"
                    ...
                    if inpars["EPSPEAK_LIN"] > 0:
                        maxP = gauss[2][i] * (
                            1.0 + inpars["EPSPEAK_LIN"]
                        )  # EPSPEAK_LIN in fraction
                        minP = gauss[2][i] / (1.0 + inpars["EPSPEAK_LIN"])
                    else:
                        maxP = np.inf
                        minP = 0.0

                    params.add(
                        "g_" + name + complab[j] + "_Peak",
                        value=gauss[2][i] / (3 * j + 1),
                        vary=True,
                        min=minP,
                        max=maxP,
                    )  # The i'th component is initialized to have an amplitude = main component / (3*j+1)
                    if parobj_update:
                        if (
                            params["g_" + name + complab[j] + "_Peak"].value
                            != parobj_update[
                                "g_" + name + complab[j] + "_Peak"
                            ].value
                        ):
                            if j == 0:
                                ...
                                raise ValueError(
                                    "Gauss params and parobj_update peaks do not concide"
                                )  # ipdb.set_trace()
                            elif j == 1:
                        ...
                        if (
                            ...
                            == 0
                            and params["g_" + name + complab[j] + "_Peak"].vary
                            != parobj_update[
                                "g_" + name + complab[j] + "_Peak"
                            ].vary
                            and init_parobj != False
                        ):
                            params["g_" + name + complab[j] + "_Peak"].value = (
                                init_parobj[
                                    "g_" + name + complab[j] + "_Peak"
                    ...
                    if name in list(doublets.keys()):
                        params["g_" + name + complab[j] + "_Peak"].set(
                            expr="g_"
                            + doublets[name][0]
                            + complab[j]
                            + "_Peak*"
        ...
        for i in range(drude[0].size):
            if drude[1][i] > 0 or get_all == True or parobj_update != False:
                ...
                if inpars["EPSWAVE0_PAH"] > 0:
                    maxW = drude[0][i] * (
                        1.0 + inpars["EPSWAVE0_PAH"] / 2.998e5
                    )
                    minW = drude[0][i] / (
                        1.0 + inpars["EPSWAVE0_PAH"] / 2.998e5
                    )
                ...
                if parobj_update:
                    ...
                    # pass
                    if (
                        params["d" + name + "_Wave"].value
                        != parobj_update["d" + name + "_Wave"].value
                    ):
                        raise ValueError(
                            "Drude params and parobj_update wavelengths do not concide"
                        )  # ipdb.set_trace()

                if inpars["EPSGAMMA_PAH"] > 0:
                    maxG = drude[1][i] * (
                        1.0 + inpars["EPSGAMMA_PAH"]
                    )  # EPSGAMMA_PAH in fraction
                    if init_parobj != False:
                        minG = np.min(
                            [
                                init_parobj["d" + name + "_Gamma"].value,
                                drude[1][i] / (1.0 + inpars["EPSGAMMA_PAH"]),
                            ]
                        )
                    else:
                        minG = np.max(
                ...
                params.add(
                    "d" + name + "_Gamma",
                    value=drude[1][i],
                    vary=inpars["FITGAMMA_PAH"],
                    min=minG,
                    max=maxG,
                )
                if parobj_update:
                    if (
                        params["d" + name + "_Gamma"].value
                        != parobj_update["d" + name + "_Gamma"].value
                    ):
                        raise ValueError(
                            "Drude params and parobj_update gammas do not concide"
                        )  # ipdb.set_trace()

                if inpars["EPSPEAK_PAH"] > 0:
                    maxP = drude[2][i] * (
                        1.0 + inpars["EPSPEAK_PAH"]
                    )  # EPSPEAK_PAH in fraction
                    minP = drude[2][i] / (1.0 * inpars["EPSPEAK_PAH"])
                else:
                    maxP = np.inf
                    minP = 0.0
                ...
                if parobj_update:
                    if (
                        params["d" + name + "_Peak"].value
                        != parobj_update["d" + name + "_Peak"].value
                    ):
                        raise ValueError(
                            "Drude params and parobj_update peaks do not concide"
                        )  # ipdb.set_trace()
                    if (
                        parobj_update["d" + name + "_Peak"].value == 0
                        and params["d" + name + "_Peak"].vary
                        != parobj_update["d" + name + "_Peak"].vary
                        and init_parobj != False
                    ):
                        params["d" + name + "_Peak"].value = init_parobj[
                            "d" + name + "_Peak"
                        ].value
        ...
        for i in range(gauss_opc[0].size):
            if gauss_opc[1][i] > 0 or get_all == True or parobj_update != False:
                ...
                if inpars["EPSWAVE0_OPC"] > 0:
                    maxW = gauss_opc[0][i] * (
                        1.0 + inpars["EPSWAVE0_OPC"] / 2.998e5
                    )  # EPSWAVE0_OPC in [km/s]
                    minW = gauss_opc[0][i] / (
                        1.0 + inpars["EPSWAVE0_OPC"] / 2.998e5
                    )
                else:
                ...
                if parobj_update:
                    ...
                    # pass
                    if (
                        params["o_" + name + "_Wave"].value
                        != parobj_update["o_" + name + "_Wave"].value
                    ):
                        raise ValueError(
                            "Opc params and parobj_update wavelengths do not concide"
                        )  # ipdb.set_trace()

                ### Do widths (gammas) ###
                if inpars["EPSGAMMA_OPC"] > 0:
                    maxG = gauss_opc[1][i] * (
                        1.0 + inpars["EPSGAMMA_OPC"]
                    )  # EPSGAMMA_OPC in fraction
                    if init_parobj != False:
                        minG = np.min(
                            [
                                init_parobj["o_" + name + "_Gamma"].value,
                                gauss_opc[1][i]
                                / (1.0 + inpars["EPSGAMMA_OPC"]),
                            ]
                        )
                ...
                params.add(
                    "o_" + name + "_Gamma",
                    value=gauss_opc[1][i],
                    vary=inpars["FITGAMMA_OPC"],
                    min=minG,
                    max=maxG,
                )  # The i'th component is initialized to have a width 2*j+1 * instrumental FWHM
                if parobj_update:
                    if (
                        params["o_" + name + "_Gamma"].value
                        != parobj_update["o_" + name + "_Gamma"].value
                    ):
                        raise ValueError(
                            "Opc params and parobj_update gammas do not concide"
                        )  # ipdb.set_trace()
                ...
                if parobj_update:
                    if (
                        params["o_" + name + "_Peak"].value
                        != parobj_update["o_" + name + "_Peak"].value
                    ):
                        raise ValueError(
                            "Opc params and parobj_update peaks do not concide"
                        )  # ipdb.set_trace()
                    if (
                        parobj_update["o_" + name + "_Peak"].vary == 0
                        and params["o_" + name + "_Peak"].vary
                        != parobj_update["o_" + name + "_Peak"].vary
                        and init_parobj != False
                    ):
                        params["o_" + name + "_Peak"].value = init_parobj[
                            "o_" + name + "_Peak"
                        ].value
        ...
        params.add(
            "VGRAD",
            value=1.0,
            vary=inpars["FITVGRAD"],
            min=-1.0 * inpars["EPSVGRAD"],
            max=1.0 * inpars["EPSVGRAD"],
        )
        if parobj_update:
            ...
            params["VGRAD"].min = (
                parobj_update["VGRAD"].value - 1.0 * inpars["EPSVGRAD"]
            )
            params["VGRAD"].max = (
                parobj_update["VGRAD"].value + 1.0 * inpars["EPSVGRAD"]
            )
...
class CAFE_prof_generator:
    ...
    def __init__(self, spec, inpars, inopts, phot_dict):
        ...
        self.T_bb = np.geomspace(3.0 + 30.0, 1750.0 + 30.0, num=30) - 30.0
        ...
        if instnames[0] == "PHOTOMETRY":
            wave = np.geomspace(
                wave[0],
                wave[-1],
                num=int(np.ceil(np.log10(wave[-1] / wave[0]) * 250)),
            )  # This is equivalent to R~100

        # Average sampling of the spectrum
        eqRspec = wave.size / np.log10(
            np.nanmax(wave) / np.nanmin(wave)
        )  # Equivalent to R
        ...
        if wave[0] >= 1.5:
            undersampNIR = (
                4.0  # = 4 times worse than the average sampling of the spectrum
            )
            minNIR = 1.5  # 1. # originally 0.3
            maxNIR = wave[0] - (wave[1] - wave[0])
            nNIR = int(
                np.ceil(np.log10(maxNIR / minNIR) * eqRspec / undersampNIR)
            )
        ...
        if wave[-1] <= 30.0:
            ...
            maxMIR = 30.0
            nMIR = int(
                np.ceil(np.log10(maxMIR / minMIR) * eqRspec / undersampMIR)
            )
        ...
        if phot_dict is not None:
            if phot_dict["wave"][0] < waveMod[0]:
                ...
                maxOpt = np.nanmax(
                    (
                        waveMod[0] - (waveMod[1] - waveMod[0]),
                        phot_dict["wave"][0],
                    )
                )
                nOpt = int(
                    np.ceil(np.log10(maxOpt / minOpt) * eqRspec / undersampOpt)
                )
            ...
            if phot_dict["wave"][-1] > waveMod[-1]:
                ...
                minFIR = np.nanmin(
                    (
                        waveMod[-1] + (waveMod[-1] - waveMod[-2]),
                        phot_dict["wave"][-1],
                    )
                )
                maxFIR = phot_dict["wave"][-1] + phot_dict["width"][-1]
                nFIR = int(
                    np.ceil(np.log10(maxFIR / minFIR) * eqRspec / undersampFIR)
                )
        ...
        for i in range(waveMod.size - 1):
            if waveMod[i + 1] - waveMod[i] < 1e-14:
                close_wave_sampling.append(i)
    ...
    def load_grain_emissivity(self):
        ...
        scaleOHMc = np.genfromtxt(
            os.path.join(tablePath, "opacity/ohmc_scale.txt"), comments=";"
        )
        ...
        if "ISRF" in sourceTypes:
            E_ISRF = grain_emissivity(
                waveMod, T_bb, "ISRF", scaleOHMc, tablePath
            )
        if "SB2Myr" in sourceTypes:
            E_SB2Myr = grain_emissivity(
                waveMod, T_bb, "SB2Myr", scaleOHMc, tablePath
            )
        if "SB10Myr" in sourceTypes:
            E_SB10Myr = grain_emissivity(
                waveMod, T_bb, "SB10Myr", scaleOHMc, tablePath
            )
        if "SB100Myr" in sourceTypes:
            E_SB100Myr = grain_emissivity(
                waveMod, T_bb, "SB100Myr", scaleOHMc, tablePath
            )

        if srcs["SOURCE_HOT"] == "AGN":
            E_HOT = E_AGN
        elif srcs["SOURCE_HOT"] == "ISRF":
            E_HOT = E_ISRF
        elif srcs["SOURCE_HOT"] == "SB2Myr":
            E_HOT = E_SB2Myr
        elif srcs["SOURCE_HOT"] == "SB10Myr":
            E_HOT = E_SB10Myr
        elif srcs["SOURCE_HOT"] == "SB100Myr":
            E_HOT = E_SB100Myr
        else:
            raise ValueError("Invalid Source_HOT")
        if srcs["SOURCE_WRM"] == "AGN":
            E_WRM = E_AGN
        elif srcs["SOURCE_WRM"] == "ISRF":
            E_WRM = E_ISRF
        elif srcs["SOURCE_WRM"] == "SB2Myr":
            E_WRM = E_SB2Myr
        elif srcs["SOURCE_WRM"] == "SB10Myr":
            E_WRM = E_SB10Myr
        elif srcs["SOURCE_WRM"] == "SB100Myr":
            E_WRM = E_SB100Myr
        else:
            raise ValueError("Invalid Source_WRM")
        if srcs["SOURCE_COO"] == "AGN":
            E_COO = E_AGN
        elif srcs["SOURCE_COO"] == "ISRF":
            E_COO = E_ISRF
        elif srcs["SOURCE_COO"] == "SB2Myr":
            E_COO = E_SB2Myr
        elif srcs["SOURCE_COO"] == "SB10Myr":
            E_COO = E_SB10Myr
        elif srcs["SOURCE_COO"] == "SB100Myr":
            E_COO = E_SB100Myr
        else:
            raise ValueError("Invalid Source_COO")
        if srcs["SOURCE_CLD"] == "AGN":
            E_CLD = E_AGN
        elif srcs["SOURCE_CLD"] == "ISRF":
            E_CLD = E_ISRF
        elif srcs["SOURCE_CLD"] == "SB2Myr":
            E_CLD = E_SB2Myr
        elif srcs["SOURCE_CLD"] == "SB10Myr":
            E_CLD = E_SB10Myr
        elif srcs["SOURCE_CLD"] == "SB100Myr":
            E_CLD = E_SB100Myr
        else:
            raise ValueError("Invalid Source_CLD")
        if srcs["SOURCE_CIR"] == "AGN":
            E_CIR = E_AGN
        elif srcs["SOURCE_CIR"] == "ISRF":
            E_CIR = E_ISRF
        elif srcs["SOURCE_CIR"] == "SB2Myr":
            E_CIR = E_SB2Myr
        elif srcs["SOURCE_CIR"] == "SB10Myr":
            E_CIR = E_SB10Myr
        elif srcs["SOURCE_CIR"] == "SB100Myr":
            E_CIR = E_SB100Myr
        else:
    ...
    def load_grain_opacity(self):
        ...
        if not inopts["SWITCHES"]["ORION_H2O"]:
            kIce3 = load_opacity(
                waveMod,
                os.path.join(
                    tablePath, "opacity/ice_opacity_idl_3um_upsampled.txt"
                ),
            )
        else:
            kIce3 = load_opacity(
                waveMod,
                os.path.join(tablePath, "opacity/ice_opacity_idl_orion.txt"),
            )
        kIce6 = load_opacity(
            waveMod,
            os.path.join(
                tablePath, "opacity/ice_opacity_idl_6um_upsampled.txt"
            ),
        )
        kHac = load_opacity(
            waveMod,
            os.path.join(tablePath, "opacity/hac_opacity_upsampled.txt"),
        )
        kCOrv = load_opacity(
            waveMod,
            os.path.join(tablePath, "opacity/corv_opacity_upsampled.txt"),
        )
        kCO2 = load_opacity(
            waveMod, os.path.join(tablePath, "opacity/CO2_opacity_4um.ecsv")
        )
        kCrySi_233 = load_opacity(
            waveMod,
            os.path.join(tablePath, "opacity/crystallineSi_opacity_233.ecsv"),
        )

        # Temperature is set to 0 to get grain opacities
        kAbs, kExt = grain_opacity(
            waveMod, 0.0, scaleOHMc, tablePath, noPAH=False
        )  # , cutoff='big'
    ...
    def get_sed(self):
        ...
        source2Myr = sourceSED(
            waveMod, "SB2Myr", tablePath, norm=True, Jy=True
        )[1]
        source10Myr = sourceSED(
            waveMod, "SB10Myr", tablePath, norm=True, Jy=True
        )[1]
        source100Myr = sourceSED(
            waveMod, "SB100Myr", tablePath, norm=True, Jy=True
        )[1]
    ...
    def make_cont_profs(self):
        ...
        flux0 = {
            "CIR": np.interp(np.log(wave0["CIR"]), np.log(wave), flux),
        ...
        cont_profs = {
            ...
            "wave0": wave0,
            "flux0": flux0,  # reference wavelengths and fluxes at those waves of continuum features
            "E_CIR": E_CIR,
            "E_COO": E_COO,
            "E_CLD": E_CLD,
            "E_WRM": E_WRM,
            "E_HOT": E_HOT,  # dust emissivities
            "kIce3": kIce3,
            "kIce6": kIce6,
            "kHac": kHac,
            "kCOrv": kCOrv,
            "kCO2": kCO2,  # molecules and non-amorphous dust opacity
            ...
            "source2Myr": source2Myr,
            "sourceDSK": sourceDsk,
            "sourceSTR": sourceStr,  # SED
            #'pwaves':pwave, 'filters':pfilt, # photometry info
            "pwaves": None,
            "filters": None,  # tmp - Don't consider photometry
...
class CAFE_cube_generator:
    """
    Class to generate parameter cube and profile cube.
    ...
    def __init__(self, cafe_obj):
        ...
        self.ny = cafe_obj.ny  # cube[extract].header['NAXIS2']
        self.nz = cafe_obj.nz
        self.z = cafe_obj.z

        if hasattr(cafe_obj, "parcube"):
            self.parcube = cafe_obj.parcube
    ...
    def make_parcube(self, params):
        ...
        for name in name_list:
            globals()[name] = fits.ImageHDU(
                ini_parcube.copy(), name=name, header=self.cube_header
            )
        ...
        expr_rec = np.rec.array(
            expr_tbl, formats="int16, U24", names="idx, expr"
        )
        ...
        parname_rec = np.rec.array(
            parname_tbl, formats="int16, U32", names="idx, parname"
        )
    ...
    def make_profcube(self, inparfile, optfile, prof_names):
        ...
        if hasattr(self, "parcube") is False:
            raise AttributeError(
                "The CAFE object does not have a parameter cube (parcube) yet. "
                "The spectrum has not been fitted or a preexisting parameter "
                "cube has not been loaded. Please use cafe.read_parcube_file() "
                "to load it from disk or use cafe.fit_spec() to fit a spectrum."
            )

        self.wave, self.flux, self.flux_unc, self.bandname, self.mask = (
            self.cube._mask_spec()
        )
        self.weight = 1.0 / self.flux_unc**2
        self.spec = Spectrum1D(
            spectral_axis=self.wave * u.micron,
            flux=self.flux * u.Jy,
            uncertainty=StdDevUncertainty(self.flux_unc),
            redshift=self.z,
        )
        ...
        for prof_name, prof_pars in prof_names.items():
            globals()[prof_name] = fits.ImageHDU(
                ini_profcube.copy(), name=prof_name, header=self.cube_header
            )
            ...
            for i in range(self.nx):
                for j in range(self.ny):
                    ...
                    if prof_pars[0] == "Comp":
                        profcube[prof_name].data[:, j, i] = CompFluxes[
                            prof_pars[1]
                        ]
                        ...
                        ind = [
                            idx
                            for idx, s in enumerate(params.keys())
                            if prof_pars[1] in s
                        ]
                        if len(ind) != 3:
                            raise ValueError(
                                "More than one feature found. Make sure the name of the feature is unique"
                            )

                        if prof_pars[1][0] == "g" or prof_pars[1][0] == "o":
                            profcube[prof_name].data[:, j, i] = gauss_prof(
                                waveMod,
                                [
                                    [param[prof_pars + "_Wave"].value],
                                    [param[prof_pars + "_Gamma"].value],
                                    [param[prof_pars + "_Peak"].value],
                                ],
                                ext=extComps["extPAH"],
                            )
                        elif prof_pars[1][0] == "d":
                            profcube[prof_name].data[:, j, i] = drude_prof(
                                waveMod,
                                [
                                    [param[prof_pars + "_Wave"].value],
                                    [param[prof_pars + "_Gamma"].value],
                            ...
                            ValueError(
                                "The feature you are trying to get the profile from is not among the fitted parameters"
                            )
    ...
    def make_map(self, parname):
        ...
        try:
            ind = (
                self.parcube["PARNAME"].data["parname"].tolist().index(parname)
            )  # find the index (parameter) that matches the parname in the z dimension of the cube
        except:
            if "Flux" or "Sigma" or "FWHM" in parname:
                try:
                    ind = (
                        self.parcube["PARNAME"]
                        .data["parname"]
                        .tolist()
                        .index(parname.replace(parname.split("_")[-1], "Wave"))
                    )
                except:
                    # ipdb.set_trace()
                    raise ValueError(
                        "Parameter is not in parameter cube. Use s.parcube['PARNAME'].data['parname'] to list the available parameters"
                    )
            else:
                raise ValueError(
                    "Parameter is not in parameter cube or 'Flux', 'Sigma', or 'FWHM'. Use s.parcube['PARNAME'].data['parname'] to list the available parameters"
                )
        ...
        if "Flux" in parname:
            if parname[0] == "g" or parname[0] == "o":
                fluxmap = (
                    np.sqrt(2.0 * np.pi)
                    * const.c.to(u.micron / u.s)
                    * (self.parcube["VALUE"].data[ind + 2] * u.Jy)
                    * self.parcube["VALUE"].data[ind + 1]
                    / 2.35482
                    / (self.parcube["VALUE"].data[ind] * u.micron)
                )
            elif parname[0] == "d":
                fluxmap = (
                    np.pi
...
def parobj2parcube(parobj, parcube, x=0, y=0):
    ...
    for parname in parobj.keys():
        ...
        vary = parobj[
            parname
        ].vary  # will be 0 if vary is False or 1 if it's True
...
def parcube2parobj(parcube, x=0, y=0, init_parobj=None):
    ...
    Turn parameters in parcube at a specific spaxel into a Parameter object
        ...
        If init_parobj is provided the parcube parameters that are in init_parobj will be injected in parobj
    ...
    if init_parobj is None:
        ...
        for z, parname in enumerate(parcube["PARNAME"].data["parname"]):
            if np.isnan(parcube["VALUE"].data[z, y, x]) == False:
                ...
                stderr = (
                    None
                    if np.isnan(parcube["STDERR"].data[z, y, x])
                    else parcube["STDERR"].data[z, y, x]
                )
                vary = False if parcube["VARY"].data[z, y, x] == 0.0 else True
                expr = (
                    None
                    if parcube["EXPR"].data["expr"][z] == "None"
                    else parcube["EXPR"].data["expr"][z]
                )
        ...
        for z, parname in enumerate(parcube["PARNAME"].data["parname"]):
            try:
                ...
                if ~np.isnan(parcube["VALUE"].data[z, y, x]):
                    ...
                    parobj[parname].stderr = (
                        None
                        if np.isnan(parcube["STDERR"].data[z, y, x])
                        else parcube["STDERR"].data[z, y, x]
                    )
                    parobj[parname].vary = (
                        False if parcube["VARY"].data[z, y, x] == 0.0 else True
                    )
                    parobj[parname].expr = (
                        None
                        if parcube["EXPR"].data["expr"][z] == "None"
        ...
        if count != len(parobj.keys()):
            # ipdb.set_trace()
            raise ValueError(
                "Some parameters in the parcube are missing in the ini_parobj"
            )
...
def parcube2df(parcube, x, y):
    ...
    df = pd.DataFrame(
        {
            "parname": list(parcube["PARNAME"].data["parname"]),
            "value": value_list,
            "stderr": stderr_list,
            "min": min_list,
            "max": max_list,
