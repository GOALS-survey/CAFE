...
from creta.paths import get_package_data_path
...
#%%
    ...
    def singleExtraction(self, data_path, parfile_path, output_path=None, parfile_name='single_params.txt',
        ...
        if PSFs_path is None:
            from creta.paths import get_psf_path
            PSFs_path = get_psf_path()


        # Read the parameter file
        if parameter_file:
            #params = user.loadUserParams(parfile_name) #Load User Parameters
            params = user.read_inipars(os.path.join(parfile_path, parfile_name)) #Load User Parameters
        ...
        if aperture_correction:
            for i in range(len(PSF_all)):
                ...
                filename = os.path.join(get_package_data_path("centroids"), "xys_"+PSF_all[i].name_band+".csv")                
                # PSF Centroids
                ...
                # INF Fluxes
                PSF_inf_filename = os.path.join(get_package_data_path("PSF_infaps"), "inf_"+PSF_all[i].name_band+".csv")
        ...
        for j in range(len(realData_all[0].rs[0])):          #for every aperture radius
            ...
            plt.savefig(os.path.join(output_path, file_naming+'_spectra.png'), dpi=385)
            #plt.show()
            plt.close()
        ...
        for aperi in range(len(dfs_alls)):
            ...
            outparfile_name = os.path.join(output_path, filenames_alls[aperi]+'_params_file.txt')
            shutil.copyfile(os.path.join(parfile_path, parfile_name), outparfile_name)
            ...
            output_file_name = os.path.join(output_path, filenames_alls[aperi]+".fits")
...
#%%
    ...
    def gridExtraction(self, data_path, parfile_path, output_path=None, parfile_name='grid_params.txt',
        ...
        if parameter_file:
            grid_params = userAPI.read_inipars(os.path.join(parfile_path, parfile_name))
        ...
        for i in range(len(files_sort)):
            cube =preprocess.getFITSData(os.path.join(data_path, files_sort[i]), silent=True)
            cubes.append(cube)
        ...
        if aperture_correction:   
            for i in range(len(PSF_all)):
                ...
                filename = os.path.join(get_package_data_path("PSF_centroids"), "xys_"+PSF_all[i].name_band+".csv")                   
                print(filename)
                if os.path.isfile(filename):
                    if i == 0: print('Loading PSF XY Centers')
                    PSF_all[i].xys = user.readCubeCentroids(filename) #read PSF centroids from file
            ...
            # Center grid to PSF
            for i in range(len(PSF_all)):
                PSF_sky_cnt_filename =  os.path.join(get_package_data_path("PSF_centroids_sky"), "sky_"+PSF_all[i].name_band+".csv")
        ...
        plt.savefig(os.path.join(output_path, file_naming+"_spectra.png"), dpi=500)
        #plt.show()
        plt.close()
        ...
        output_file_name = os.path.join(output_path, file_naming+".fits")
        ...
        if parameter_file:
            ...
            outparfile_name_grid = os.path.join(output_path, file_naming+"_params_file.txt")
            shutil.copyfile(os.path.join(parfile_path, parfile_name), outparfile_name_grid)
