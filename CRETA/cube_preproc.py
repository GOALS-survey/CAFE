...
class cube_preproc:
    ...
    def getFITSData(self, cube_file, silent=False):
        ...
        pixelScale = (
            np.sqrt(
                hdu_list["SCI"].header["CDELT1"]
                * hdu_list["SCI"].header["CDELT2"]
            )
            * 3600
        )  # arcsec / pixel
        if hdu_list["PRIMARY"].header["INSTRUME"] == "NIRSPEC":
            ...
            cube_name = (
                "ch"
                + str(hdu_list["PRIMARY"].header["CHANNEL"])
                + "_"
                + str(hdu_list["PRIMARY"].header["BAND"])
            )
        ...
        if "MJD-BEG" in headers:
            del headers[10:16]
        if "REFFRAME" in headers:
            del headers[12:30]
    ...
    def getPSFData(self, cube_file):
        ...
        if hdu_list[instkey].header["INSTRUME"] == "NIRSPEC":
            ...
            cube_name = (
                "ch"
                + str(hdu_list[instkey].header["CHANNEL"])
                + "_"
                + str(hdu_list[instkey].header["BAND"])
            )
    ...
    def getChannelLs(self, subcube):
        ...
        for i in range(subcube.cube_before.shape[0]):
            l_i = self.getSimulatedL(
                subcube.CRPIX3, subcube.CRVAL3, subcube.CDELT3, i
            )
            all_li.append(l_i)
    ...
    def getPointSourceRs(self, subcube, base_r):
        ...
        for i in range(subcube.cube_before.shape[0]):
            ...
            r_i = self.getRpixPointSource(
                base_r, subcube.pixel_scale, subcube.ls[i], subcube.base_l
            )
            all_ri.append(r_i)
    ...
    def getPSFPixelCenters(self, subcube, image):
        ...
        xx, yy, zz = subcube.wcs.world_to_pixel(
            c1, subcube.ls[0] * u.um
        )  #### TDS
        res = []
    ...
    def AperturePhotometry(self, subcube, image):
        ...
        for i in range(image.shape[0]):
            ...
            [photometries, errors, areas] = self.getApertureSum(
                jj,
                kk,
                i,
                subcube.rs[i],
                sliceIm,
                subcube.wcs,
                subcube.error_data[i, :, :],
                subcube.aperture_type,
                subcube.DQ,
            )
    ...
    def getApertureSum(self, x, y, z, r, data, w, error, aprture_type, DQ):
        ...
        for i in range(len(r)):
            ...
            if aprture_type == 0:
                ...
                aper = RectangularAperture([x, y], r[i] * 2.0, r[i] * 2.0)
            apers.append(aper)

        w_2d = w.celestial
        phot_table = aperture_photometry(data, apers, wcs=w_2d, error=error)
        rs_photometry = []
        rs_error = []
        rs_area = []
        for i in range(len(r)):
    ...
    def imageCentroid(self, image):
        ...
        if (
            np.isnan(np.sum(zoom2img))
            or np.isinf(np.sum(zoom2img))
            or np.sum(zoom2img) <= 0.0
        ):
            ...
            cx, cy = centroid_2dg(
                zoom2img
            )  # task from phot utils will return x,y
            xx = start_X + xx - 5 + cx
            yy = start_Y + yy - 5 + cy
    ...
    def userCentroid(self, image, x, y, hbox_x=5, hbox_y=5):
        ...
        zoom2img = image[
            iy - hbox_y : iy + hbox_y + 1, ix - hbox_x : ix + hbox_x + 1
        ]
        ...
        if (
            ...
            print(
                "The cetroid algorithm failed because there are some nans or infs or negative values in the images around the centering region"
            )
            return [x, y]
    ...
    def lambdaBasedCentering(self, cubes, user_ra, user_dec, l_c, dxdy=False):
        ...
        if len(res_cubes) == 0:
            raise ValueError(
                "The wavelength for centering is not within the wavelength range of the cubes provided."
            )
        ...
        plane = np.nansum(
            the_cube.cube_before[
                np.max(((z - 2), 0)) : np.min(
                    ((z + 3), len(the_cube.cube_before))
                ),
                :,
                :,
            ],
            axis=0,
        )  # add one before and one plane after l_c

        xx, yy = self.userCentroid(plane, x, y)

        print(
            "Centering around", the_cube.ls[z], "um in cube", the_cube.name_band
        )
    ...
    def getSubcubesAllAppended(self, subcubes, background):
        ...
        for i in range(len(subcubes)):
            ...
            if background:
                all_background.append(subcubes[i].background_spectrum)
    ...
    def createGridInArcSec(
        ...
        if r_ap == -1:
            print(
                "Warning: The spaxel size is not defined. Using as default the distance between spaxels."
            )
    ...
    def plotGrid(
        ...
        for i in range(len(pixels_list)):
            ...
            plt.gca().add_patch(
                Rectangle(
                    [
                        sky_list[i].ra.value - r_as / 3600,
                        sky_list[i].dec.value - r_as / 3600,
                    ],
                    2 * r_as / 3600,
                    2 * r_as / 3600,
                    linewidth=1,
        ...
        plt.legend()
        plt.savefig(
            output_path + output_filebase_name + "_" + cube.name_band + ".png"
        )
        # plt.show()
        plt.close()
    ...
    def stitchingRatioCalculation(
        self, realData, aperture_correction, idx, grid
    ):
        # 'G140H',
        ...
        for i in range(len(realData) - 1):
            ...
            ratio = self.calculateStitchRatio(
                realData[i], realData[i + 1], aperture_correction, idx, grid
            )
    ...
    def calculateStitchRatios(self, realData, aperture_correction, idx, grid):
        ...
        for i in range(len(realData) - 1):
            ...
            if aperture_correction:
                ...
                if grid:
                    chA_data = np.array(
                        chA.spectrum_PSF_corrected
                    )  # not tested
                    chB_data = np.array(
                        chB.spectrum_PSF_corrected
                    )  # not tested
                else:
            ...
            chB_stop = np.where(
                np.array(chB_ls) <= chA_ls[len(chA_ls) - 1] + delta1 / 2
            )[0]
    ...
    def stitchSpectrum(self, ratio_list, idx, spectrum):
        ...
        firstone = np.where(np.array(ratio_list[idx:]) == 1.0)[0][0]
