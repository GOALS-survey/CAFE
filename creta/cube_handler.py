...
class cube_handler:
    ...
    def __init__(
        ...
        self.CDELT1_pix = self.head_keys[
            "CDELT1"
        ]  # First axis increment per pixel
        self.CDELT2_pix = self.head_keys[
            "CDELT2"
        ]  # Second axis increment per pixel
        ...
        self.total_flux_before = self.preprocess.totalImageFlux(
            self.cube_before
        )
        self.base_r = base_r
        self.base_l = base_l
        ...
        if convolve:
            ...
            if isPSF:
                if os.path.isfile(seff_filename):
                    ...
                    self.psf_sigma_eff = self.readSigmaEffective(
                        seff_filename
                    )  # read PSF centroids from file
                else:
    ...
    def doFluxUnitConversion(self):
        ...
        self.corrected_spectrum = (
            np.array(self.apers) * 10**6 * (delta_factor / 206265**2)
        )  # Convert flux spectrum to Jy
        self.delta_factor_list.append(delta_factor)
        self.delta_factor_list * len(self.rs)
        self.error = (
            np.array(self.error) * 10**6 * (delta_factor / 206265**2)
        )  # Convert error spectrum to Jy
    ...
    def doCenters(self, user_ra, user_dec, isPSF, perband_cent):
        ...
        if isPSF:
            self.xys = self.preprocess.getPSFPixelCenters(
                self, self.cube_before
            )
        else:
            if perband_cent is False:
                self.xys = self.preprocess.getPixelCenters(
                    self, self.cube_before
                )
            else:
                print(
                    "Per-band re-centering around",
                    self.ls[2],
                    "um in cube",
                    self.name_band,
                )
                self.xys = self.preprocess.getPixelCentersPerCube(
                    self, self.cube_before
                )
                sky = self.wcs.pixel_to_world(
                    self.xys[0][0], self.xys[0][1], self.ls[2]
                )
    ...
    def doSinglePhotometry(self, isPSF, backg):
        ...
        if backg:
            [self.apers, self.area, self.error] = (
                self.preprocess.AperturePhotometry(self, self.cube_after)
            )
        else:
            [self.apers, self.area, self.error] = (
                self.preprocess.AperturePhotometry(self, self.cube_before)
            )
        ...
        if isPSF:
            ...
            for i in range(len(self.apers[1])):
                self.PSF_correction.append(
                    np.array(
                        np.array(self.PSF_inf_flux) / np.array(self.apers)[:, i]
                    ).T
                )
        ...
        print(
            self.name_band
            + " photometry exectued in: %s seconds"
            % (time.time() - time_photometry)
        )
        # %%
    ...
    def doGridPhotometry(self, sky_ra, sky_dec, r_arcsec, data, plot):
        ...
        aperture = RectangularAperture(positions, r_pix * 2.0, r_pix * 2.0)
        res = []
        DQ_phot = []
        ...
        for i in range(len(data)):
            phot_table = aperture_photometry(
                ...
                aperture,
                wcs=self.wcs,
                error=self.error_data[i, :, :],
            )
            DQ_phot_table = aperture_photometry(
                self.DQ[i, :, :], aperture, wcs=self.wcs
            )
    ...
    def subtractBackground(self, r_in, width):
        ...
        (
            ...
            self.med_sigma,
            self.annulus,
            self.annulus_centroid,
            self.annulus_aperture,
            self.routs,
        ) = self.preprocess.subtractUserBackground(self, r_in, r_out)
        ...
        print(
            "+++++ Background Subtraction Time: %s seconds +++++"
            % (time.time() - time_background)
        )
    ...
    def PSFCorrection(self, ratio, PSF_ls):
        ...
        for i in range(len(self.apers[0])):
            ...
            model.fit(
                np.array(PSF_ls)[valinds[0]].reshape(-1, 1),
                np.array(ratio)[i, valinds[0]].reshape(-1, 1),
            )
            self.spectrum_PSF_corrected.append(
                (
                    np.array(self.corrected_spectrum)[:, i]
                    * model.predict(np.array(self.ls).reshape(-1, 1)).reshape(
                        -1
                    )
                )
            )  ### TDS
            self.error_PSF_corrected.append(
                (
                    np.array(self.error)[:, i]
                    * model.predict(np.array(self.ls).reshape(-1, 1)).reshape(
                        -1
                    )
                )
    ...
    def sliceGridExtractionPhotometry(
        self, sky_ra, sky_dec, r_arcsec, data, plot, idx
    ):
        r_pix = r_arcsec / self.pixel_scale
        # print("r pix is ", r_pix)
        positions = []
        ...
        DQ_phot_table = aperture_photometry(
            self.DQ[idx, :, :], aperture, wcs=self.wcs
        )
        res.append(phot_table)
        DQ_phot.append(DQ_phot_table["aperture_sum"])
        aps = [aperture] * len(data)
        ...
        for i in range(len(self.xys)):
            self.sky_xys.append(
                self.wcs.pixel_to_world(
                    self.xys[i][0], self.xys[i][1], self.ls[i]
                )
            )
    ...
    def plotApertures(
        self, background, user_rs_arcsec, output_path, output_filebase_name
    ):
        ...
        for i in range(len(self.rs[0])):
            ...
            apers.append(aper)
            ap_patches = aper.plot(
                color=colors[i], lw=2, label="Photometry aperture_" + str(i)
            )
            patches.append(ap_patches[0])

        if background:
            ann_patches = self.annulus_aperture[0].plot(
                color="red", lw=2, label="Background annulus"
            )
    ...
    def plotInformationMask(self, i):
        ...
        ap_patches_inf = aper_inf.plot(
            color="green", lw=2, label="Inf aperture"
        )
        ann_patches = self.annulus_aperture[i].plot(
            color="red", lw=2, label="Background annulus"
        )
    ...
    def plotGrid(self, pixels, r, x, y, cubes):
        ...
        for i in range(len(pixels)):
            ...
            plt.gca().add_patch(
                Rectangle(
                    [xx, yy], r, r, linewidth=1, edgecolor="r", facecolor="none"
                )
            )
    ...
    def convolve_subband(self, PSF):
        ...
        for waveind in range(nwaveinds):
            ...
            subframe = np.asarray(
                frame[
                    int(np.rint(nys / 2)) - 10 : int(np.rint(nys / 2)) + 10,
                    int(np.rint(nxs / 2)) - 10 : int(np.rint(nxs / 2)) + 10,
                ]
            )
            ...
            peakind = tuple(
                map(
                    sum,
                    zip(
                        np.where(subframe == np.nanmax(subframe)),
                        [
                            int(np.rint(nys / 2)) - 10,
                            int(np.rint(nxs / 2)) - 10,
                        ],
                    ),
                )
            )
            ...
            if waveind == 0:
                ...
                params.add("par0", value=peak[0], min=0.0, max=np.inf)
                # Initial position of the peak
                params.add(
                    "par1", value=peakind[1][0]
                )  # X coordinate pixel where the peak of the PSF is
                params.add(
                    "par2", value=peakind[0][0]
                )  # Y coordinate pixel where the peak of the PSF is
                params.add("par3", value=1.5)
                params.add("par4", value=1.0)
                params.add("par5", value=np.pi / 4.0, min=0.0, max=2.0 * np.pi)
            else:
            ...
            gauss2d_fit = mylmfit2dfun.mylmfit2dfun(
                xyflat, frame[valinds], params, zerr=frame_err[valinds]
            )
            ...
            self.psf_sigma_eff.append(
                np.sqrt(gauss2d_fit.pars[3] * gauss2d_fit.pars[4])
                * current_channel_pixel_scale
            )  # Units of arcsecond
    ...
    def fixConvolved(self, reference_psf_sigma_eff, psf_sigma_eff_list):
        ...
        for waveind in range(len(self.ls)):
            ...
            kernel_sigma = (
                np.sqrt(
                    reference_psf_sigma_eff**2
                    - psf_sigma_eff_list[waveind] ** 2
                )
                / self.pixel_scale
            )
            ...
            self.cube_before[waveind, :, :] = convolve(
                frame,
                gauss2d_kernel,
            ...
            self.error_data[waveind, :, :] = convolve(
                frame_err,
                gauss2d_kernel,
                normalize_kernel=True,
                nan_treatment="interpolate",
