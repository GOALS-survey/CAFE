...
def gauss_prof(wave, gauss, ext=None):
    ...
    unitgauss = np.exp(gausspts**2 / -2.0)
...
def drude_prof(wave, drude, ext=None):
    ...
    """
    drude_arr = np.asarray(
        drude[:3]
    ).transpose()  # get drude arr with 3 cols: wave, width, peak
    ...
    for i in range(len(drude_arr)):  # add each line
        pah_prof += (
            peak[i]
            * wid[i] ** 2
            / ((wave / lam[i] - lam[i] / wave) ** 2 + wid[i] ** 2)
        )
...
def drude_int_fluxes(
    wave, drude, ext=None, scale=1.0, flxunits=u.Jy, wvunits=u.um
):
    ...
    for i in range(len(int_fluxes)):
        ...
        totflux += flux
        ### Units
        flux = (flux * flxunits).to(
            u.W / u.m**2 / wvunits, equivalencies=u.spectral_density(wave)
        )
        ### Integrate
