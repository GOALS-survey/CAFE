...
def sourceSED_ISRF(wave, T_bb=None):
    ...
    uWave[((wave > 0.0912) & (wave < 0.11))] = (
        38.57 / 3e10 * np.power(wave[((wave > 0.0912) & (wave < 0.11))], 3.4172)
    )
    uWave[((wave > 0.11) & (wave < 0.134))] = 2.045e-2 / 3e10
    uWave[((wave > 0.134) & (wave < 0.246))] = (
        7.115e-4
        / 3e10
        * np.power(wave[((wave > 0.134) & (wave < 0.246))], -1.6678)
    )
    ...
    if T_bb is not None:
        ...
        tBBOld = np.power(3e10 / 4.0 / sigma_SB * uTotOld, 0.25)
        uWave *= (T_bb / tBBOld) ** 4
...
def sourceSED_AGN(wave, lTot=1e11, r=None, T_bb=None):
    ...
    # 0.1216 < wave/um < 10
    idx = wave > 0.1216
    lFreq[idx] = np.interp(0.1216, wave, lFreq) * np.power(
        wave[idx] / 0.1216, 0.46
    )
    # 10 < wave/um < 1e3
    bb = planck(wave, 1e3, um=False, Hz=True)
    idx = wave > 10.0
    lFreq[idx] = np.interp(10.0, wave, lFreq) * (
        bb[idx] / np.interp(10.0, wave, bb)
    )
...
def sourceSED_SB(wave, age, tablePath, nebular=False):
    ...
    if age == "2":
        fWave[wave > 70.0] *= np.exp(-1.0 * (wave[wave > 70.0] - 70.0) / 70.0)
