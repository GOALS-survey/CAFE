...
def cafeplot(
    ...
    CLD_TMP = (
        ""
        if params == None
        else r" (" + "{:.0f}".format(params["CLD_TMP"].value) + "$\,$K" + ")"
    )
    COO_TMP = (
        ""
        if params == None
        else r" (" + "{:.0f}".format(params["COO_TMP"].value) + "$\,$K" + ")"
    )
    WRM_TMP = (
        ""
        if params == None
        else r" (" + "{:.0f}".format(params["WRM_TMP"].value) + "$\,$K" + ")"
    )
    HOT_TMP = (
        ""
        if params == None
        else r" (" + "{:.0f}".format(params["HOT_TMP"].value) + "$\,$K" + ")"
    )
    ...
    ax1.tick_params(axis="y", labelsize=12)
    ax1.set_ylim(
        bottom=0.1 * np.nanmin(min_flux), top=2.0 * np.nanmax(max_flux)
    )
