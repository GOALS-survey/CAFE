...
class cafe_io:
    ...
    @staticmethod
    def pah_table(parcube, contcube, x=0, y=0, parobj=False, 
                  pah_complex=True, pah_obs=False, write2disk=False):
        """
        ...
        pah_obs: bool (default: False)
            ...
            based on the measured extinction. If set to True, output will be the 
        ...
        for n in pah_name:
            ...
            # Flux estimate
            # -------------
            lam = p.filter(like='Wave', axis=0).value[0] * u.micron
            ...
            # Calculate the "observed" PAH flux, which is calculated without the need of extinction correction.
            # In this case, the derived PAH flux should always be smaller or equal than the extinction corrected PAH flux.
            
            wave = contcube['CompFluxes']['wave']

            ext_scale = np.interp(lam.value, wave, contcube['extComps']['extPAH'])
            ...
            # -------------------------
            _lam_unc = p.filter(like='Wave', axis=0).stderr[0]
            ...
            # Continuum profile
            I_nu_C = contcube['CompFluxes']['fCON'] # The straightout continuum flux level, without extinction correction

            # PAH profile
            #I_nu_P = I_nu_C + contcube['CompFluxes']['fPAH']
        ...
        # EQW measurement
        # ===============
        # define the function which will be used later to find PAH bands that are closest to the limits
